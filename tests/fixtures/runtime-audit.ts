...
export type RuntimeAuditEvidence = {
  ...
  checks: {
    horizontalOverflow: 'absent' | 'logged'; clippedOverlay: 'absent' | 'logged'; hiddenAction: 'absent' | 'logged'
    keyboardObstruction: 'absent' | 'logged'; gestureTrap: 'absent' | 'logged'; landscape: 'recorded' | 'not-applicable' | 'missing'
  }
...
async function attachAuditScreenshot(page: Page, testInfo: TestInfo) {
  ...
  try {
    await testInfo.attach('runtime-audit-screenshot', { body: await page.screenshot({ fullPage: true }), contentType: 'image/png' })
  } catch { /* browser crashes can make a screenshot unavailable; validation reports that explicitly */ }
}
...
export const test = base.extend<AuditFixtures>({
  blockedMutationRequests: [async ({ context, baseURL, page }, use, testInfo) => {
    ...
    await context.route('**/*', async route => {
      ...
      if (new URL(request.url()).origin === auditedOrigin && !['GET', 'HEAD', 'OPTIONS'].includes(request.method())) {
...
export async function auditRenderedPage(page: Page, testInfo: TestInfo, route: string, state: RuntimeAuditState, viewport: AuditViewport, fixtureIssues: RuntimeAuditIssue[] = []) {
  ...
  const collectLayout = () => page.evaluate(() => {
    ...
    const clippedActions = Array.from(document.querySelectorAll('a[href], button, input, select, textarea, [role="button"]')).filter(el => {
      if (!visible(el) || el.hasAttribute('disabled') || !clipped(el)) return false
      const overlay = el.closest(overlaySelector)
      return ['fixed', 'sticky'].includes(getComputedStyle(el).position) || Boolean(overlay && visible(overlay))
    }).map(label)
    const mapCanvas = Array.from(document.querySelectorAll('.maplibregl-canvas')).find(visible)
    const mapControls = Array.from(document.querySelectorAll('.maplibregl-ctrl button')).filter(visible).length
  ...
  try { layout = await collectLayout() } catch (first) {
    await page.waitForLoadState('domcontentloaded').catch(() => undefined)
    try { layout = await collectLayout() } catch (second) {
      const size = page.viewportSize()
      layout = { horizontalOverflow: false, scrollWidth: size?.width ?? viewport.width, viewportWidth: size?.width ?? viewport.width, overlays: [], clippedActions: [], gestureSurfaceWithoutControls: false }
      issues.push({ category: 'state-fixture', details: `Layout evidence could not be collected after navigation settled: ${second instanceof Error ? second.message : String(second)} (initial: ${first instanceof Error ? first.message : String(first)})` })
    }
  }
  if (layout.horizontalOverflow) issues.push({ category: 'horizontal-overflow', details: `document width ${layout.scrollWidth}px exceeds ${layout.viewportWidth}px viewport` })
  for (const value of layout.overlays) issues.push({ category: 'clipped-overlay', details: value })
  for (const value of layout.clippedActions) issues.push({ category: 'hidden-action', details: value })
  if (layout.gestureSurfaceWithoutControls) issues.push({ category: 'gesture-trap', details: 'Map gesture surface disables native touch gestures without visible map controls' })

  let keyboard: RuntimeAuditEvidence['checks']['keyboardObstruction'] = 'absent'
  const focusTarget = page.locator('input:not([type="hidden"]), textarea, select').filter({ visible: true }).first()
  if (await focusTarget.count()) {
    const size = page.viewportSize()
    if (size) {
      await page.setViewportSize({ width: size.width, height: Math.max(360, Math.floor(size.height * 0.58)) })
      await focusTarget.focus(); await focusTarget.scrollIntoViewIfNeeded()
      if (await focusTarget.evaluate(el => el.getBoundingClientRect().bottom > window.innerHeight)) { keyboard = 'logged'; issues.push({ category: 'keyboard-obstruction', details: 'The focused form control is below the simulated keyboard viewport' }) }
      await page.setViewportSize(size)
    }
  }
  const evidence: RuntimeAuditEvidence = { project: testInfo.project.name, route, state, viewport, checks: {
    horizontalOverflow: issues.some(i => i.category === 'horizontal-overflow') ? 'logged' : 'absent', clippedOverlay: issues.some(i => i.category === 'clipped-overlay') ? 'logged' : 'absent', hiddenAction: issues.some(i => i.category === 'hidden-action') ? 'logged' : 'absent', keyboardObstruction: keyboard, gestureTrap: layout.gestureSurfaceWithoutControls ? 'logged' : 'absent', landscape: viewport.orientation === 'landscape' ? 'recorded' : 'not-applicable' }, issues }
  await testInfo.attach('runtime-audit-evidence', { body: JSON.stringify(evidence), contentType: 'application/json' })
  const applicationFinding = issues.some(i => i.category !== 'state-fixture')
  if (applicationFinding && ((viewport.width >= 320 && viewport.width <= 430) || viewport.orientation === 'landscape')) await attachAuditScreenshot(page, testInfo)
...
async function installGeolocationFixture(page: Page, state: 'success' | 'error' | 'timeout') {
  await page.addInitScript(fixtureState => {
    const position = { coords: { accuracy: 10, altitude: null, altitudeAccuracy: null, heading: null, latitude: 51.096, longitude: 0.225, speed: null }, timestamp: Date.now() }
    const failure = { code: fixtureState === 'timeout' ? 3 : 1, message: fixtureState === 'timeout' ? 'Audit fixture timed out' : 'Audit fixture denied permission', PERMISSION_DENIED: 1, POSITION_UNAVAILABLE: 2, TIMEOUT: 3 }
    Object.defineProperty(navigator, 'geolocation', { configurable: true, value: { clearWatch: () => undefined, getCurrentPosition: (success: PositionCallback, error?: PositionErrorCallback | null) => setTimeout(() => fixtureState === 'success' ? success(position as GeolocationPosition) : error?.(failure as GeolocationPositionError), fixtureState === 'timeout' ? 100 : 0), watchPosition: () => 1 } })
  }, state)
}
...
export async function exerciseRuntimeState(page: Page, context: BrowserContext, state: Exclude<RuntimeAuditState, 'default'>) {
  ...
  if (state === 'webgl-failure') await page.addInitScript(() => { const original = HTMLCanvasElement.prototype.getContext; HTMLCanvasElement.prototype.getContext = function (contextId, options) { if (contextId === 'webgl' || contextId === 'webgl2' || contextId === 'experimental-webgl') return null; return original.call(this, contextId, options) } as typeof HTMLCanvasElement.prototype.getContext })
  if (state === 'map-resource-failure') await page.route(/tiles\.openfreemap\.org\/.+\.(?:pbf|png|webp)(?:\?.*)?$/, route => route.abort('connectionfailed'))
  if (state === 'pin-request-failure') await page.route(/\/api\/crags\/pins(?:\?.*)?$/, route => route.abort('connectionfailed'))
  if (state.startsWith('geolocation-')) await installGeolocationFixture(page, state.replace('geolocation-', '') as 'success' | 'error' | 'timeout')
