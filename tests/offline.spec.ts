...
test.describe('mandatory offline reliability harness', () => {
  ...
  test('installs online and opens the complete saved guide across an offline page restart', async ({ page, context }) => {
    ...
    const traverseRoute = reopened.locator('[data-route-line-id="55555555-5555-4555-8555-555555555552"]')
    await expect(curvedRoute.locator('path')).toHaveCount(3)
    await expect(curvedRoute.locator('path').first()).toHaveAttribute('d', 'M 0.2 0.9 Q 0.4 0.2 0.5 0.4 Q 0.6 0.6 0.7 0.4 L 0.8 0.2')
    await expect(curvedRoute.locator('path').nth(2)).toHaveAttribute('stroke', '#00FFFF')
    ...
    await reopened.getByRole('button', { name: 'Airplane Traverse · 6A' }).click()
    await expect(curvedRoute.locator('path').nth(2)).toHaveAttribute('stroke', '#10b981')
    await expect(traverseRoute.locator('path').nth(2)).toHaveAttribute('stroke', '#00FFFF')
