...
function TestHarness({ onCommit, ...props }: { activeDraftImageId: string | null; existingRouteLines: RouteLine[]; routesByImageId: Record<string, DraftRoute[]>; setRoutesByImageId: React.Dispatch<React.SetStateAction<Record<string, DraftRoute[]>>>; routeType: string; onCommit?: (commit: () => unknown) => void }) {
  const { commitRoutes } = useEditDraftRouteStoreSync(props)
...
describe('useEditDraftRouteStoreSync', () => {
  ...
  it('seeds the store and clears canvas UI on image switch', () => {
    ...
    rerender(<TestHarness activeDraftImageId="image-2" existingRouteLines={secondRoutes} routesByImageId={{}} setRoutesByImageId={vi.fn()} routeType="boulder" />)
