...
describe('crag pack manifest builder', () => {
  it('filters public content, applies coordinate policy, and emits immutable fixed-format assets', () => {
    const result = buildCragPackManifest(source(), 'https://static.example/')
  ...
  it('has deterministic content versions independent of input order and generatedAt', () => {
    ...
    const first = buildCragPackManifest(firstSource, 'https://static.example')
    const second = buildCragPackManifest(secondSource, 'https://static.example')
  ...
  it('omits legacy, malformed, and private media and rejects inactive crags', () => {
    ...
    expect(buildCragPackManifest({ ...input, crag: crag({ deleted_at: '2026-01-01' }) }, 'https://static.example')).toBeNull()
  })
})
