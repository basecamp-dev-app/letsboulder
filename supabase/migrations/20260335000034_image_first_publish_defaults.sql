...
DECLARE
  ...
  metadata_version INTEGER := COALESCE((draft_row.metadata->>'version')::INTEGER, 1);
...
BEGIN
  ...
  IF draft_row.status = 'submitted' THEN
    default_live_image_id := NULLIF(COALESCE(draft_row.metadata->>'publishedImageId', draft_row.metadata->>'image_id', ''), '')::UUID;
  ...
  IF metadata_version >= 2 THEN
    default_draft_image_id := NULLIF(COALESCE(draft_row.metadata->'navigation'->>'defaultImageId', ''), '')::UUID;
  ELSE
    SELECT di.id
    ...
    WHERE di.draft_id = draft_row.id
    ORDER BY di.display_order
    OFFSET GREATEST(COALESCE((draft_row.metadata->>'primaryIndex')::INTEGER, 0), 0)
    LIMIT 1;
  END IF;
  ...
  IF default_draft_image_id IS NULL THEN
    RAISE EXCEPTION 'Default draft image is missing';
  END IF;

  route_type_default := NULLIF(btrim(COALESCE(draft_row.metadata->>'routeType', draft_row.metadata->'submission'->>'routeType', '')), '');
  ...
  IF metadata_version >= 2 THEN
    IF jsonb_typeof(COALESCE(draft_row.metadata->'images'->(default_draft_image_id::TEXT)->'orientation', 'null'::JSONB)) <> 'array'
      OR jsonb_array_length(COALESCE(draft_row.metadata->'images'->(default_draft_image_id::TEXT)->'orientation', '[]'::JSONB)) = 0 THEN
      RAISE EXCEPTION 'Default image orientation is required before publishing';
    END IF;
  END IF;
