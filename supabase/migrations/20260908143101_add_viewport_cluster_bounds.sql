-- Return the actual member bounds for server-generated viewport clusters so the
-- client can fit the map to the cluster contents instead of guessing an
-- expansion zoom. Clustering remains enabled only through zoom 11; zoom 12+
-- continues to return leaf rows only.
...
-- CREATE OR REPLACE, so recreate the public/admin wrappers and internal helper
-- with the four appended nullable cluster-bound columns.
DROP FUNCTION IF EXISTS public.get_admin_viewport_map_features(
  double precision, double precision, double precision, double precision, integer
);
DROP FUNCTION IF EXISTS public.get_viewport_map_features(
  double precision, double precision, double precision, double precision, integer
);
DROP FUNCTION IF EXISTS public.get_viewport_map_features_internal(
...
BEGIN
  ...
  SELECT
    ...
    count(*)::bigint,
    CASE WHEN p_zoom <= 11 AND count(*) > 1 THEN min(b.longitude)::double precision ELSE NULL::double precision END,
    CASE WHEN p_zoom <= 11 AND count(*) > 1 THEN min(b.latitude)::double precision ELSE NULL::double precision END,
    CASE WHEN p_zoom <= 11 AND count(*) > 1 THEN max(b.longitude)::double precision ELSE NULL::double precision END,
    CASE WHEN p_zoom <= 11 AND count(*) > 1 THEN max(b.latitude)::double precision ELSE NULL::double precision END
  FROM bucketed b
  GROUP BY
    b.grid_x,
    b.grid_y,
