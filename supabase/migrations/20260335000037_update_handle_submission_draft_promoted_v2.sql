-- Fix handle_submission_draft_promoted to support both v1 (metadata.location) and v2 (metadata.submission.location) structures
...
BEGIN
  IF NEW.status = 'submitted' AND OLD.status = 'draft' THEN
    -- Check v2 location (metadata.submission.location) first
    ...
    -- Fallback to v1 location (metadata.location) if v2 is not found
    IF draft_latitude IS NULL THEN
    ...
    IF draft_longitude IS NULL THEN
