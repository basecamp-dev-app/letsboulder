...
AS $function$
  WITH base_climbs AS (
    SELECT
      ...
      climbs.route_type
    FROM public.climbs
    WHERE climbs.crag_id = p_crag_id
      AND climbs.status IN ('active', 'approved')
  ),
  route_meta AS (
    SELECT
      base_climbs.id AS climb_id,
      COUNT(DISTINCT route_lines.image_id)::int AS topo_image_count
    FROM base_climbs
  ...
  route_direction_values AS (
    ...
    WHERE images.face_direction IS NOT NULL
    ...
    WHERE face_direction.direction IS NOT NULL
