...
(function () {
  ...
  const STORAGE_KEY = 'offline-climb-packs';
  ...
  function renderPacks(packs) {
    ...
    if (!packs.length) {
      ...
      subtitleEl.textContent = 'No saved climbs found on this device yet.';
      return;
    }

    emptyEl.hidden = true;
    subtitleEl.textContent = packs.length === 1
      ? '1 climb is ready to open offline.'
      : packs.length + ' climbs are ready to open offline.';

    listEl.innerHTML = packs.map(function (pack) {
      return [
        '<a href="/climb/' + encodeURIComponent(pack.climbId) + '" class="block rounded-2xl border border-gray-200 bg-white px-4 py-4 shadow-sm transition hover:border-gray-300 hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-900 dark:hover:border-gray-700 dark:hover:bg-gray-800">',
        '<div class="flex items-start justify-between gap-4">',
        '<div>',
        '<p class="text-base font-semibold text-gray-900 dark:text-gray-100">' + escapeHtml(pack.climbName || 'Saved climb') + '</p>',
        '<p class="mt-1 text-sm text-gray-500 dark:text-gray-400">/climb/' + escapeHtml(pack.climbId) + '</p>',
        '</div>',
        '<span class="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-medium text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-200">Offline ready</span>',
        '</div>',
        '<div class="mt-3 flex items-center justify-between text-sm text-gray-500 dark:text-gray-400">',
        '<span>' + Number(pack.mediaCount || 0) + ' photos</span>',
      ...
      getRequest.onsuccess = function () {
        ...
        const packs = raw && typeof raw === 'object'
          ? Object.values(raw).sort(function (a, b) {
              return String(a.climbName || '').localeCompare(String(b.climbName || ''));
            })
          : [];
        renderPacks(packs);
      };
      ...
      <div className="mx-auto max-w-2xl">
        <div className="rounded-3xl border border-white/70 bg-white/90 p-6 shadow-xl shadow-emerald-950/5 backdrop-blur dark:border-white/10 dark:bg-gray-950/80 dark:shadow-black/30">
          ...
          <div id="offline-empty" className="mt-8 rounded-2xl border border-dashed border-gray-300 bg-gray-50 px-5 py-6 text-sm text-gray-600 dark:border-gray-700 dark:bg-gray-900/70 dark:text-gray-300" hidden>
            No saved climbs found on this device yet.
          </div>
          ...
          <div className="mt-8 rounded-2xl border border-gray-200 bg-gray-50 px-4 py-4 text-sm text-gray-600 dark:border-gray-800 dark:bg-gray-900/70 dark:text-gray-300">
            Open any saved climb to view its topo photos and core climb data while offline.
          </div>
        </div>
      </div>
