...
const saveStarterRoutesSchema = z.object({
...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!floorPlan) {
      return NextResponse.json({ floor_plan: null, routes: [] })
    }
    ...
    return NextResponse.json({ floor_plan: floorPlan, routes: items })
  } catch (error) {
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const parsedBody = parseWithSchema(saveStarterRoutesSchema, await request.json())
