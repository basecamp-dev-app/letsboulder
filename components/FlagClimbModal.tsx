...
const FLAG_TYPES = [
  ...
  { value: 'wrong_crag', label: 'Wrong Crag', description: 'Climb is linked to the wrong crag' },
...
export default function FlagClimbModal({ climbId, climbName, onClose, onSubmitted }: FlagClimbModalProps) {
  useOverlayHistory({ open: true, onClose, id: `flag-climb-${climbId}` })

  const [step, setStep] = useState<'select' | 'comment'>('select')
  const [flagType, setFlagType] = useState('')
  const [selectedType, setSelectedType] = useState<typeof FLAG_TYPES[number] | null>(null)
  const [comment, setComment] = useState('')
  const [submitting, setSubmitting] = useState(false)
  const [error, setError] = useState('')
  const [success, setSuccess] = useState(false)

  const handleTypeSelect = (type: typeof FLAG_TYPES[number]) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (trimmedComment.length < MIN_COMMENT_LENGTH) {
      setError(`Please provide at least ${MIN_COMMENT_LENGTH} characters of detail`)
      return
    }
        ...
        >
          <div className="text-center">
            ...
            <p className="text-gray-600 dark:text-gray-400">
              Thank you. An admin will review {climbName}.
            </p>
          </div>
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} className="p-4">
            ...
            <div className="mb-4">
              ...
              <div className="text-right text-xs text-gray-500 dark:text-gray-400 mt-1">
                {comment.length}/{MAX_COMMENT_LENGTH} characters (minimum {MIN_COMMENT_LENGTH})
              </div>
            </div>
            ...
            <div className="flex gap-3">
              ...
              <button
                type="submit"
                disabled={submitting || comment.trim().length < MIN_COMMENT_LENGTH}
