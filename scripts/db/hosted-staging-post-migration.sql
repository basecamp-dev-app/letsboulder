-- Hosted-only verification. Structural checks are read-only. The visibility
-- probe uses one synthetic review crag and always rolls it back.
DO $verification$
DECLARE
...
BEGIN
  ...
  PERFORM * FROM public.resolve_public_crag_slug('ZZ', 'hosted-staging-verification-missing');
  PERFORM * FROM public.resolve_public_climb_slug('ZZ', 'hosted-staging-verification-missing', 'missing');
END
$verification$;

BEGIN;
CREATE TEMP TABLE hosted_staging_visibility_probe (
  crag_id uuid PRIMARY KEY
) ON COMMIT DROP;
GRANT SELECT ON hosted_staging_visibility_probe TO anon;

CREATE TEMP TABLE hosted_staging_visibility_result (
  review_visible boolean NOT NULL
) ON COMMIT DROP;
GRANT INSERT ON hosted_staging_visibility_result TO anon;

WITH inserted AS (
  INSERT INTO public.crags (id, name, slug, country_code, latitude, longitude)
  VALUES (
    extensions.gen_random_uuid(),
    'Hosted staging review visibility probe',
    'hosted-staging-review-visibility-probe',
    'GB',
    51.000001,
    -1.000001
...
INSERT INTO hosted_staging_visibility_probe (crag_id)
SELECT id FROM inserted;

SET LOCAL ROLE anon;
INSERT INTO pg_temp.hosted_staging_visibility_result (review_visible)
SELECT EXISTS (
  SELECT 1 FROM public.crags
  WHERE id IN (SELECT crag_id FROM pg_temp.hosted_staging_visibility_probe)
);
RESET ROLE;

DO $visibility$
BEGIN
  IF (SELECT review_visible FROM pg_temp.hosted_staging_visibility_result) THEN
    RAISE EXCEPTION 'Review crag was visible to anon';
  END IF;
END
$visibility$;
ROLLBACK;
...
SELECT jsonb_build_object(
  ...
  'metrics_rpc', public.get_public_impact_metrics_v1()
) AS hosted_staging_verification;
