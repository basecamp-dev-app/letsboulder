...
export const starterRouteSchema = z.object({
...
export const saveStarterRoutesSchema = z.object({
  routes: z.array(starterRouteSchema).optional().default([]),
})
...
export async function loadStarterRoutes(
  ...
  try {
    ...
    if (!floorPlan) {
      return NextResponse.json({
        floor_plan: null,
        routes: [],
        ...(membershipRole !== undefined && { membership_role: membershipRole }),
      })
    }
...
export async function saveStarterRoutes(
  supabase: SupabaseClient,
  gymId: string,
  routes: StarterRoute[]
) {
...
export function parseSaveStarterRoutesRequest(body: unknown) {
  return parseWithSchema(saveStarterRoutesSchema, body)
}
