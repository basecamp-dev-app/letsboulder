...
import { ArrowLeft, MapPinned, Mountain, RefreshCw, Route, Trash2 } from 'lucide-react'
                ...
                return (
                  <li key={pack.packId} className="overflow-hidden rounded-3xl border border-stone-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
                    <div className="flex h-28 items-center justify-center bg-emerald-50 text-emerald-800 dark:bg-emerald-950/30 dark:text-emerald-300">
                      {pack.kind === 'crag' ? <Mountain aria-hidden="true" /> : <Route aria-hidden="true" />}
                    </div>
                    <div className="p-5">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className="rounded-full bg-stone-100 px-2.5 py-1 text-xs capitalize text-stone-600 dark:bg-gray-800 dark:text-gray-300">{pack.status}</span>
                      </div>
                      <p className="mt-3 text-xs text-stone-500 dark:text-gray-400">{ready ? `Installed ${pack.installedAt ? new Date(pack.installedAt).toLocaleDateString() : 'on this device'}` : pack.error || 'Finishing download'}</p>
                      {pack.kind === 'crag' && ready ? (
                        ...
                        <Button className="mt-5 w-full rounded-xl" disabled>{ready ? 'Standalone route viewer unavailable' : 'Not ready'}</Button>
                      )}
                      {ready ? (
