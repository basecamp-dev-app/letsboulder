...
- **Shell releases:** Bump the shell cache suffix when changing the offline shell contract. The build manifest versions static assets using the Next asset paths and the contents of both MapLibre worker modules. `sw.js` imports generated `/sw-build-version.js`, so a new build triggers a worker update and retains its cache identity across offline restarts. Installation must fully cache required shells, discovered Next assets, and `/maplibre/maplibre-gl-worker.mjs` plus `/maplibre/maplibre-gl-shared.mjs` before activation removes the preceding owned release cache. Stable MapLibre URLs are read only from the current build cache, never a previous release cache.
- **Offline navigation:** `/offline`, `/offline/library`, and `/offline/crag` must resolve from the shell cache before any network request. `/offline` renders the library directly; connectivity verification is informational and must never gate saved content. The worker retains its build version in its imported script and stores the manifest in the shell cache; cached application chunks and MapLibre modules remain readable after an offline worker restart.
