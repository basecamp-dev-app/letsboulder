...
5. The Worker then performs an anonymous `GET` through the production media hostname and requires a non-empty image response. Only `verify_media_replacement_delivery(...)` records that proof on the deletion job. Unverified source-replacement jobs cannot be claimed, and canonical ingest never deletes the source directly.
...
Production maintenance uses `media-reconciliation.yml` to classify every R2 object across authoritative locators, draft/crag references, ingest jobs, and deletion jobs. `media-canonical-migration.yml` accepts only live candidates from a fresh reconciliation, transforms at most 25 sources per run with Sharp, verifies public delivery before and after the atomic switch, and restores the captured database state if post-commit delivery fails. Neither workflow directly deletes an R2 object.
