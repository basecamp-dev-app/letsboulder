...
- **Release check identity** — The gate reads only `push` runs on `main` for the selected SHA from `test.yml` and `security.yml`, then validates mandatory jobs in each latest run's exact attempt. Deployment-status, scheduled, and manually dispatched CI runs cannot authorize a release. Missing or skipped mandatory jobs fail closed; use **Re-run all jobs** after a partial rerun. Behavioral regression tests cover unrelated runs, failed attempts, missing jobs, pagination, and attempts changing during validation.
- **Basemap smoke** — `tests/map-basemap.spec.ts` is included in `@smoke`; it requires HTTP 200 tile responses, nonblank rendered canvas pixels, and successful higher-zoom tiles after zooming. Run it against staging before promotion.
