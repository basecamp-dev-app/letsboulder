...
4. [`useEditDraftActions`](../features/draft-editor/hooks/use-edit-draft-actions.ts) performs an explicit Save through one `save_submission_draft_atomic` transaction. The RPC checks `expected_updated_at`, replaces every submitted dirty image's durable route set, updates the complete image order and compatibility `route_data`, merges metadata/location, updates the crag/editor identity, and returns one new revision. Route editing is therefore explicit-save, not debounced autosave.
...
- The legacy route-sync endpoint remains image-scoped last-write-wins for compatibility and publication repair. Explicit Save does not use it: dirty route replacements, image state, metadata, and crag selection commit or roll back together. Upload attachment has its own one-retry conflict loop.
