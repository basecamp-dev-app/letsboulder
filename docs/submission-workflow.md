...
- One published save creates a commit grouping changed image, climb, and route-line snapshots. Existing entities have a baseline parent, patches are generated from authoritative database snapshots, and a repeated mutation UUID returns the original commit without adding revisions. Owner credit/anonymity and crag metadata paths also append image/crag revisions.
