...
| `grade_mappings` | Grade-to-points lookup table |
...
| `claim_media_job_for_image(worker_name, image_id)` | Service-only targeted claim for Queue wake-ups |
...
Linked database commands are maintainer deployment operations, not part of the local development workflow. After a verified `staging → main` promotion, the **Production Release** workflow validates the exact current `main` SHA, the protected production Supabase target, mandatory release checks, and a production migration dry-run. If the remote migration history is current, the code-only application release continues automatically. If migrations are pending or cannot be proven current, the automatic run stops before Vercel; a maintainer manually dispatches **Production Release** with the exact current `main` SHA, which repeats validation, applies and verifies migrations, rechecks `main`, and only then triggers the single production application deploy path. Production releases are serialized.
