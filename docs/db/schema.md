...
- `submission_draft_routes` stores durable draft route geometry and metadata. `save_submission_draft_atomic` replaces submitted dirty route sets together with the complete image order, compatibility `route_data`, merged metadata/location, crag, editor identity, and one concurrency-checked draft revision.
